...
def set_background_image(image_path=None):
    """Set the app background.

    Uses a local image (blurred, cover-fit) when `image_path` points to a
    file that actually exists. Originally this was always called with a
    plain description string ("Abstract red to black gradient background
    with grainy noise texture...") instead of a real file path, which
    crashed the entire app on every load with FileNotFoundError. It now
    degrades gracefully to a pure-CSS red-to-black gradient (matching that
    same intended look) whenever no usable image file is available.
    """
    image_data = None
    if image_path:
        try:
    ...
    if image_data:
        background_layer = f'background-image: url("data:image/png;base64,{image_data}"); background-size: cover; background-position: center; background-repeat: no-repeat; background-attachment: fixed;'
    else:
        background_layer = (
            "background-image: radial-gradient(circle at 20% 20%, "
            "rgba(120,0,0,0.35), transparent 60%), "
            "linear-gradient(160deg, #2b0000 0%, #1a0000 35%, #0a0a0a 70%, #000000 100%); "
...
# ============================================================================
# PREPARE DATASET (canonical - was dead code, now wired up)
# ============================================================================
...
# ============================================================================
# GLOBAL FILTERS (previously never called; CSS re-attached to function)
# ============================================================================
...
# ============================================================================
# MARKET & CHANNEL ANALYSIS (fuller of the two duplicates)
# ============================================================================
...
# ============================================================================
# EXECUTIVE OVERVIEW (full version - was shadowed dead code)
# ============================================================================
...
# ============================================================================
# HOTEL PERFORMANCE (detailed version - was never wired)
# ============================================================================
...
# ============================================================================
# BOOKING TRENDS (detailed version - was never wired)
# ============================================================================
...
# ============================================================================
# CANCELLATION ANALYSIS (full version - was shadowed dead code)
# ============================================================================
...
# ============================================================================
# LEAD TIME ANALYSIS (full version - was shadowed dead code)
# ============================================================================
...
# ============================================================================
# STAY DURATION (full version - was shadowed dead code)
# ============================================================================
...
# ============================================================================
# SPECIAL REQUIREMENTS ANALYSIS (new page)
# ============================================================================
...
# ============================================================================
# REVENUE ANALYSIS (full version - was shadowed dead code)
# ============================================================================
...
# ============================================================================
# CUSTOMER ANALYSIS (full version - was shadowed dead code)
# ============================================================================
...
# ============================================================================
# REPEAT GUEST ANALYSIS (new page)
# ============================================================================
...
# ============================================================================
# GUEST COMPOSITION ANALYSIS (new page)
# ============================================================================
...
# ============================================================================
# GEOGRAPHIC MARKET ANALYSIS (new page)
# ============================================================================
...
# ============================================================================
# DISTRIBUTION CHANNEL ANALYSIS (new page)
# ============================================================================
...
# ============================================================================
# RELATIONSHIP / CORRELATION ANALYSIS (full version)
# ============================================================================
...
# ============================================================================
# BUSINESS INSIGHTS (full version - was a placeholder stub)
# ============================================================================
...
# ============================================================================
# RECOMMENDATIONS (full version - was a placeholder stub)
# ============================================================================
...
# ============================================================================
# DATA QUALITY ANALYSIS (new page; stats setup folded in from session_state)
# ============================================================================
...
# ============================================================================
# SIDEBAR (navigation list expanded to all 22 pages)
# ============================================================================
...
# ============================================================================
# PROCEED BUTTON VISIBILITY / PROCESSING INDICATOR
# ============================================================================
...
# ============================================================================
# PROCEED BUTTON + DATA CLEANING PIPELINE
# ============================================================================
...
# ============================================================================
# PROCESSING INDICATOR (post-cleaning)
# ============================================================================
...
if st.session_state.get("show_processing") and st.session_state.get("prep_popup_visible") and st.session_state.get("prep_popup_status") in {"success", "error"}:
    cols = st.columns([1, 2, 1])
    with cols[1]:
        close_preparation = st.button("Close", key="close_preparation_popup", width="stretch")
...
# ============================================================================
# DATA PIPELINE WIRING (the core fix)
# ============================================================================


# --------------------------------------------------------------
# DATA PIPELINE WIRING
# (previously: prepare_dataset() and render_global_filters() were
#  fully built but never called, and cleaning_stats / quality
#  stats relied on a broken globals() lookup that always returned
#  None. All four are now sourced from st.session_state.)
# --------------------------------------------------------------
...
if original_df is not None and cleaned_df is not None:
    duplicates_removed = int(original_df.duplicated().sum())
    missing_values_fixed = max(
        int(original_df.isna().sum().sum() - cleaned_df.isna().sum().sum()),
        0,
    )
    dtypes_fixed = int(
        sum(
            ...
            and str(original_df[column].dtype) != str(cleaned_df[column].dtype)
        )
    )
    cleaning_stats = {
...
# Prepare the working dataset (adds total_stay_nights / estimated_revenue,
# coerces dtypes). Falls back to an empty frame if nothing has been
# uploaded and cleaned yet - every page below already guards on
# `df is None or df.empty`.
df = prepare_dataset(cleaned_df)

# Sidebar navigation + cleaning-stat display / CSV export
...
# Apply the global filter panel (hotel / date / market segment / etc.)
filtered_df = render_global_filters(df)



# ============================================================================
# PAGE DISPATCH (single clean routing block - was duplicated x2)
# ============================================================================
...
if selected_page == "📋 Dataset Overview":
    ...
    render_data_preparation(df, original_df, cleaning_stats)

elif selected_page == "🔍 Data Quality Analysis":
    render_data_quality_analysis(filtered_df)

elif selected_page == "📊 Executive Overview":
    render_executive_overview(filtered_df)

elif selected_page == "🏨 Hotel Performance":
    render_hotel_performance_analysis(filtered_df)

elif selected_page == "📅 Booking Trends":
    render_booking_trend_analysis(filtered_df)
...
elif selected_page == "🍽️ Meal Analysis":
    render_meal_analysis(filtered_df)

elif selected_page == "📝 Special Requirements Analysis":
    render_special_requirements_analysis(filtered_df)
...
elif selected_page == "🔁 Repeat Guest Analysis":
    render_repeat_guest_analysis(filtered_df)

elif selected_page == "👪 Guest Composition Analysis":
    render_guest_composition_analysis(filtered_df)

elif selected_page == "🌍 Market & Channel Analysis":
    render_market_channel_analysis(filtered_df)

elif selected_page == "🗺️ Geographic Market Analysis":
    render_geographic_market_analysis(filtered_df)

elif selected_page == "🔀 Distribution Channel Analysis":
    render_distribution_channel_analysis(filtered_df)
