...
def apply_global_filters(df):
    ...
    with col1:
        ...
        if "hotel" in filtered_df.columns:
            ...
            filtered_df = filtered_df[
                filtered_df["hotel"].isin(selected_hotel)
            ]
    ...
    with col2:
        ...
        if "arrival_date_year" in filtered_df.columns:
            ...
            filtered_df = filtered_df[
                filtered_df["arrival_date_year"].isin(selected_year)
            ]
    ...
    with col3:
        ...
        if "arrival_date_month" in filtered_df.columns:
            ...
            # Standard hotel dataset month order
            month_order = [
                "January",
                "February",
                "March",
            ...
            filtered_df = filtered_df[
                filtered_df["arrival_date_month"].isin(selected_month)
            ]
    ...
    with col4:
        ...
        if "customer_type" in filtered_df.columns:
            ...
            filtered_df = filtered_df[
                filtered_df["customer_type"].isin(selected_customer)
            ]
    ...
    with col5:
        ...
        if "is_repeated_guest" in filtered_df.columns:
            ...
            selected_repeat = st.selectbox(
                ...
                key="global_repeat_filter"
            )

            if selected_repeat != "All":

                filtered_df = filtered_df[
                    filtered_df["is_repeated_guest"]
                    ==
                    repeat_options[selected_repeat]
                ]
    ...
    with col6:
        ...
        if "market_segment" in filtered_df.columns:
            ...
            filtered_df = filtered_df[
                filtered_df["market_segment"].isin(selected_segment)
            ]
    ...
    with col7:
        ...
        if "distribution_channel" in filtered_df.columns:
            ...
            filtered_df = filtered_df[
                filtered_df["distribution_channel"].isin(selected_channel)
            ]
    ...
    with col8:
        ...
        if "is_canceled" in filtered_df.columns:
            ...
            selected_cancellation = st.selectbox(
                ...
                key="global_cancellation_filter"
            )

            if selected_cancellation != "All":

                filtered_df = filtered_df[
                    filtered_df["is_canceled"]
                    ==
                    cancellation_options[selected_cancellation]
                ]
    ...
    with st.expander("⚙️ Advanced Filters", expanded=False):
        ...
        if "lead_time" in filtered_df.columns:
            ...
            if not lead_data.empty:
                ...
                min_lead = int(lead_data.min())
                max_lead = int(lead_data.max())

                if min_lead < max_lead:

                    selected_lead = st.slider(
                        "⏳ Lead Time (days)",
                        min_value=min_lead,
                        max_value=max_lead,
                        value=(min_lead, max_lead),
                        key="global_lead_filter"
                    )
        ...
        if "adr" in filtered_df.columns:
            ...
            if not adr_data.empty:
                ...
                min_adr = float(adr_data.min())
                max_adr = float(adr_data.max())

                if min_adr < max_adr:

                    selected_adr = st.slider(
                        "💰 ADR",
                        min_value=min_adr,
                        max_value=max_adr,
                        value=(min_adr, max_adr),
                        step=1.0,
                        key="global_adr_filter"
                    )
        ...
        if "stay_duration" in filtered_df.columns:
            ...
            if not stay_data.empty:
                ...
                min_stay = int(stay_data.min())
                max_stay = int(stay_data.max())

                if min_stay < max_stay:

                    selected_stay = st.slider(
                        "🛏️ Stay Duration (nights)",
                        min_value=min_stay,
                        max_value=max_stay,
                        value=(min_stay, max_stay),
                        key="global_stay_filter"
                    )
        ...
        if "stays_in_week_nights" in filtered_df.columns:
            ...
            if not week_data.empty:
                ...
                min_week = int(week_data.min())
                max_week = int(week_data.max())
        ...
        if "stays_in_weekend_nights" in filtered_df.columns:
            ...
            if not weekend_data.empty:
                ...
                min_weekend = int(weekend_data.min())
                max_weekend = int(weekend_data.max())

                if min_weekend < max_weekend:

                    selected_weekend = st.slider(
                        "🌴 Weekend Nights",
                        min_value=min_weekend,
                        max_value=max_weekend,
                        value=(min_weekend, max_weekend),
                        key="global_weekend_nights_filter"
                    )
        ...
        if "meal" in filtered_df.columns:
            ...
            selected_meal = st.multiselect(
                ...
                key="global_meal_filter"
            )

            filtered_df = filtered_df[
                filtered_df["meal"].isin(selected_meal)
            ]
        ...
        if "reserved_room_type" in filtered_df.columns:
            ...
            selected_room = st.multiselect(
                ...
                key="global_reserved_room_filter"
            )

            filtered_df = filtered_df[
                filtered_df["reserved_room_type"].isin(selected_room)
            ]
        ...
        if "assigned_room_type" in filtered_df.columns:
            ...
            selected_assigned_room = st.multiselect(
                "🛏️ Assigned Room Type",
                options=assigned_room_options,
                default=assigned_room_options,
                key="global_assigned_room_filter"
            )

            filtered_df = filtered_df[
                filtered_df["assigned_room_type"]
                .isin(selected_assigned_room)
            ]
        ...
        if "deposit_type" in filtered_df.columns:
            ...
            selected_deposit = st.multiselect(
                ...
                key="global_deposit_filter"
            )

            filtered_df = filtered_df[
                filtered_df["deposit_type"].isin(selected_deposit)
            ]
        ...
        if "booking_changes" in filtered_df.columns:
            ...
            if not changes_data.empty:
                ...
                min_changes = int(changes_data.min())
                max_changes = int(changes_data.max())

                if min_changes < max_changes:

                    selected_changes = st.slider(
                        "🔄 Booking Changes",
                        min_value=min_changes,
                        max_value=max_changes,
                        value=(min_changes, max_changes),
                        key="global_booking_changes_filter"
                    )
        ...
        if "days_in_waiting_list" in filtered_df.columns:
            ...
            if not waiting_data.empty:
                ...
                min_waiting = int(waiting_data.min())
                max_waiting = int(waiting_data.max())

                if min_waiting < max_waiting:

                    selected_waiting = st.slider(
                        "⏱️ Days in Waiting List",
                        min_value=min_waiting,
                        max_value=max_waiting,
                        value=(min_waiting, max_waiting),
                        key="global_waiting_filter"
                    )
        ...
        if "total_of_special_requests" in filtered_df.columns:
            ...
            if not request_data.empty:
                ...
                min_requests = int(request_data.min())
                max_requests = int(request_data.max())

                if min_requests < max_requests:

                    selected_requests = st.slider(
                        "🛎️ Special Requests",
                        min_value=min_requests,
                        max_value=max_requests,
                        value=(min_requests, max_requests),
                        key="global_special_requests_filter"
                    )
        ...
        if "required_car_parking_spaces" in filtered_df.columns:
            ...
            if not parking_data.empty:
                ...
                min_parking = int(parking_data.min())
                max_parking = int(parking_data.max())

                if min_parking < max_parking:

                    selected_parking = st.slider(
                        "🚗 Required Car Parking Spaces",
                        min_value=min_parking,
                        max_value=max_parking,
                        value=(min_parking, max_parking),
                        key="global_parking_filter"
                    )
...
with st.sidebar:
    ...
    filtered_df = apply_global_filters(prepared_df)
...
def render_business_insights_page(df):
    ...
    st.markdown(
        ...
        <p style="
            ...
            margin-bottom:30px;
        ">
            Data-driven findings generated directly from the prepared dataset.
        </p>
        """,
        unsafe_allow_html=True
    )
    ...
    if "lead_time" in data.columns:
        ...
        lead_data = data["lead_time"].dropna()
    ...
    if "hotel" in data.columns:
        ...
        if not hotel_counts.empty:
            ...
            insights.append({
                ...
                "text":
                    f"{top_hotel} received the highest number of bookings "
                    f"with {top_hotel_bookings:,} bookings, representing "
                    f"{top_hotel_share:.1f}% of all bookings in the dataset."
            })

    # ---------------------------------------------------------------------------------------------
    # 2. HOTEL CANCELLATION COMPARISON
    # ---------------------------------------------------------------------------------------------
    ...
    if (
        ...
        if len(hotel_cancel) >= 2:
            ...
            insights.append({
                "icon": "❌",
                "title": "Cancellation Difference",
                "text":
                    f"{highest_hotel} has the highest cancellation rate "
                    f"at {highest_rate:.1f}%, compared with "
    ...
    if "market_segment" in data.columns:
        ...
        if not segment_counts.empty:
            ...
            top_segment_bookings = segment_counts.iloc[0]
            ...
            insights.append({
                ...
                "text":
                    f"{top_segment} is the largest market segment with "
                    f"{top_segment_bookings:,} bookings, accounting for "
                    f"{segment_share:.1f}% of all bookings."
            })
    ...
    if (
        ...
        if not segment_cancel.empty:
            ...
            highest_segment = segment_cancel.index[0]
            highest_segment_rate = segment_cancel.iloc[0]

            insights.append({
                "icon": "⚠️",
                "title": "Highest-Risk Market Segment",
                "text":
                    f"{highest_segment} has the highest cancellation rate "
                    f"among the market segments at "
                    f"{highest_segment_rate:.1f}%."
            })
    ...
    if "distribution_channel" in data.columns:
        ...
        if not channel_counts.empty:
            ...
            top_channel_bookings = channel_counts.iloc[0]
            ...
            insights.append({
                ...
                "text":
                    f"{top_channel} generated the highest booking volume "
                    f"with {top_channel_bookings:,} bookings, representing "
                    f"{channel_share:.1f}% of all bookings."
            })
    ...
    if "arrival_date_month" in data.columns:
        ...
        if not month_counts.empty:
            ...
            highest_month_bookings = month_counts.iloc[0]

            lowest_month = month_counts.index[-1]
            lowest_month_bookings = month_counts.iloc[-1]

            insights.append({
                "icon": "📅",
                "title": "Demand Pattern",
                "text":
                    f"{highest_month} recorded the highest booking volume "
                    f"with {highest_month_bookings:,} bookings, while "
                    f"{lowest_month} recorded the lowest with "
                    f"{lowest_month_bookings:,} bookings."
            })

    # ---------------------------------------------------------------------------------------------
    # 7. LEAD TIME AND CANCELLATION
    # ---------------------------------------------------------------------------------------------
    ...
    if (
        ...
        if (
            ...
            difference = (
                long_cancel - short_cancel
            )

            if difference > 0:

                text = (
                    f"Bookings made more than the median lead time "
                    f"({median_lead:.0f} days) have a cancellation rate "
                    f"of {long_cancel:.1f}%, compared with "
                    f"{short_cancel:.1f}% for shorter lead times. "
                    f"This represents a {difference:.1f} percentage-point difference."
                )
    ...
    if "customer_type" in data.columns:
        ...
        if not customer_counts.empty:
            ...
            insights.append({
                ...
                "text":
                    f"{top_customer} is the most common customer type "
                    f"with {top_customer_count:,} bookings, representing "
                    f"{customer_share:.1f}% of all bookings."
            })
    ...
    if "is_repeated_guest" in data.columns:
        ...
        insights.append({
            ...
            "text":
                f"The dataset contains {repeat_bookings:,} repeat-guest "
                f"bookings, meaning repeat guests account for "
                f"{repeat_rate:.1f}% of total bookings."
        })
    ...
    if "stay_duration" in data.columns:
        ...
        if not stay_data.empty:
            ...
            average_stay_value = stay_data.mean()

            longest_stay = stay_data.max()
            ...
            insights.append({
                ...
                "text":
                    ...
                    f"{average_stay_value:.1f} nights, while the longest "
                    f"recorded stay in the dataset is "
                    f"{longest_stay:.0f} nights."
            })
    ...
    if (
        ...
        if not hotel_adr.empty:
            ...
            highest_adr_hotel = hotel_adr.index[0]
            highest_adr_value = hotel_adr.iloc[0]

            insights.append({
                "icon": "💰",
                "title": "ADR Performance",
                "text":
                    f"{highest_adr_hotel} has the highest average ADR "
                    f"at {highest_adr_value:.2f} in the dataset."
            })
    ...
    st.markdown("### 🔎 Findings From The Dataset")
    ...
    # Cancellation takeaway
    if cancellation_rate is not None:

        if cancellation_rate >= 30:

            cancellation_text = (
                f"The dataset shows a high cancellation level of "
                f"{cancellation_rate:.1f}%. Out of {total_bookings:,} bookings, "
                f"{cancelled_bookings:,} were cancelled."
            )

        elif cancellation_rate >= 20:

            cancellation_text = (
                f"The dataset shows a moderate cancellation level of "
                f"{cancellation_rate:.1f}%, with {cancelled_bookings:,} "
                f"cancelled bookings out of {total_bookings:,}."
            )
    ...
    if average_adr is not None:
        ...
        st.markdown(
            ...
            ">
                ...
                <br><br>
                <span style="color:#cccccc;">
                    The average daily rate recorded in the dataset is
                    {average_adr:.2f}.
                </span>
            </div>
            """,
    ...
    if average_lead_time is not None:
        ...
        st.markdown(
            ...
            ">
                ...
                <br><br>
                <span style="color:#cccccc;">
                    Customers make their bookings an average of
                    {average_lead_time:.0f} days before arrival.
                </span>
            </div>
            """,
            unsafe_allow_html=True
    ...
    if average_stay is not None:
        ...
        st.markdown(
            ...
            ">
                ...
                <br><br>
                <span style="color:#cccccc;">
                    The average booking in the dataset represents a stay
                    of {average_stay:.1f} nights.
                </span>
            </div>
            """,
            unsafe_allow_html=True
